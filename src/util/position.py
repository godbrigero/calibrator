...
class Position3D:
    def __init__(self, x: float, y: float, z: float, sin: float, cos: float, vx: float = 0, vy: float = 0, vz: float = 0):
        self.x = x
        self.y = y
        self.z = z
        self.sin = sin
        ...
        self.transformation_matrix = make_transformation_matrix_3d(np.array([self.x, self.y, self.z]), np.array([self.cos, 0, self.sin]))
